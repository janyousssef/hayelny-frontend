...
const Result = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`http://20.203.182.15:8080/images/${id}/diagnosis`);
  ...
  return (
    ...
    <div className="bg-white border-gray-400 border-2 rounded-xl py-36 pl-16 w-[981px] ">
        ...
        <div className='bg-white shadow-xl rounded-2xl h-full p-20 text-blue-400 font-semibold text-2xl'>
                <p className='mb-10 text-blue-600'>You have <span className='text-red-400'>{data?.disease}</span>  You should consult a doctor</p>
